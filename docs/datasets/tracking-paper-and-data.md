...
在 MOT16、 MOT17排名表中可以找到一些新的作品，但是最近几个月似乎没有作品开放他们的源代码，所以我没有更新源代码列表。有时，我认为 MOT 基准可能会导致研究人员走向错误的方向。MOT17基准提供了三种检测结果: DPM，faster RCNN 和 SDP，如果你分析基于不同检测的结果，你会发现大多数跟踪器在更快的 RCNN 和 SDP 检测上有相似的性能。跟踪器排名的好坏主要取决于 DPM 检测的良好性能，在采用Faster RCNN 和 SDP 检测时，跟踪器的性能与 DeepSort 类似。因此，如果你想提高你的 MOTA 分数，关注 DPM 的结果。
多目标跟踪是计算机视觉领域的一个深入研究但尚未成功解决的课题。这个领域需要更多的开放源码和更多的标准评估指标。开放源代码不是这个领域的特征(可能有大量的参数等待调整) ... ..。这是一个多目标跟踪的文件列表。
## Datasets 数据集
### Surveillance Scenarios 监控场景
[PETS2009](http://www.cvg.reading.ac.uk/PETS2009/a.html) : 一个老数据集.<br>
...
### Multi-person Face Tracking 多个人体人脸追踪
...
### Multi-person Pose Tracking  多个人体姿势追踪
...
## 参考
 [[1] https://github.com/SpyderXu/multi-object-tracking-paper-list.git](https://github.com/SpyderXu/multi-object-tracking-paper-list.git)